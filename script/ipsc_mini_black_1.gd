...
func handle_bullet_collision(bullet_position: Vector2) -> String:
	...
	if is_point_in_zone("AZone", local_pos):
		...
		points = ScoreUtils.new().get_points_for_hit_area("BlackZoneLeft", 0)
		...
		points = ScoreUtils.new().get_points_for_hit_area("BlackZoneRight", 0)
...
func initialize_bullet_hole_pool():
	...
	# --- Initialize legacy node pool (kept as fallback) ---
...
func spawn_bullet_hole(local_position: Vector2):
	...
	Uses GPU instanced MultiMesh if available, otherwise falls back to node pool.
	"""
	if bullet_hole_multimeshes.size() > 0 and bullet_hole_textures.size() > 0:
		...
		if not DEBUG_DISABLE:
			print("[ipsc_mini_black_1] Spawned instanced bullet hole idx=", texture_index, " at ", local_position)
		return

	# Fallback: legacy node pool
...
func load_bullet_hole_textures():
	...
	for i in range(bullet_hole_textures.size()):
		if not bullet_hole_textures[i]:
			print("[ipsc_mini_black_1] ERROR: Failed to load bullet hole texture ", i + 1)
