...
var upper_level_scene: String = "res://ipsc_mini_stage/ipsc_mini_stage.tscn"
...
var sub_menu_config: Dictionary = {}  # Store configuration for sub menu scenes
...
func _on_settings_loaded(_result, response_code, _headers, body):
	...
	if response_code == 200:
		...
		if json and json.has("data"):
			...
			# Emit signal to notify that settings are loaded (success or fallback)
	...
	if response_code == 200 and _result == HTTPRequest.RESULT_SUCCESS:
		...
		if json:
			...
			if json.has("code") and json["code"] != 0:
				...
				netlink_status = {}
				# print("GlobalData: netlink_status request failed with error code: ", json["code"], " - ", json.get("msg", "Unknown error"))
				# Clear netlink_status and emit signal (listeners will see it's empty)
				netlink_status = {}
