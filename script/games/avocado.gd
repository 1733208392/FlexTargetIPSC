...
func hit():
	...
	# Wait for animation to finish, then remove the avocado
...
func _on_bullet_hit(hit_pos: Vector2):
	...
	# Get the avocado's collision shape
	...
	if hit_detected:
		print("Hit detected on avocado at: ", hit_pos)
		# Trigger the avocado animation/split
		hit()
		# Disconnect from signal after being hit
		WebSocketListener.bullet_hit.disconnect(_on_bullet_hit)
	else:
		print("Hit missed avocado at: ", hit_pos)
