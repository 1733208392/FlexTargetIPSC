...
func _on_bullet_hit(hit_pos: Vector2):
	...
	# Get the banana's collision shape
	var collision_shape = get_node("CollisionShape")
	if collision_shape == null:
		return
	
	# Convert hit_pos to local coordinates of the banana
	...
	if hit_detected:
		print("Hit detected on banana at: ", hit_pos)
		# Trigger the banana animation/split
		hit()
		# Disconnect from signal after being hit
		WebSocketListener.bullet_hit.disconnect(_on_bullet_hit)
	else:
		print("Hit missed banana at: ", hit_pos)
