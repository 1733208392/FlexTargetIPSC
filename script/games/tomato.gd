...
func _ready():
	...
	# Connect to truck's truck_hit_by_fruit signal
	# Get truck from parent (game scene)
...
func hit():
	...
	var game = get_parent()
	game.add_score(12, global_position)
	
	# Wait for animation to finish, then remove the tomato
...
func _on_bullet_hit(hit_pos: Vector2):
	...
	# Get the tomato's collision shape
	var collision_shape = get_node("CollisionShape")
	if collision_shape == null:
		return
	
	# Convert hit_pos to local coordinates of the tomato
	...
	if hit_detected:
		print("Hit detected on tomato at: ", hit_pos)
		# Trigger the tomato animation/split
		hit()
		# Disconnect from signal after being hit
		WebSocketListener.bullet_hit.disconnect(_on_bullet_hit)
	else:
		print("Hit missed tomato at: ", hit_pos)
