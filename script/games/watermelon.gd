...
func hit():
	...
	var game = get_parent()
	game.add_score(15, global_position)
	
	# Wait for animation to finish, then remove the watermelon
...
func _on_bullet_hit(hit_pos: Vector2):
	...
	# Get the watermelon's collision shape
	var collision_shape = get_node("CollisionShape")
	if collision_shape == null:
		return
	
	# Convert hit_pos to local coordinates of the watermelon
	...
	if hit_detected:
		print("Hit detected on watermelon at: ", hit_pos)
		# Trigger the watermelon animation/split
		hit()
		# Disconnect from signal after being hit
		WebSocketListener.bullet_hit.disconnect(_on_bullet_hit)
	else:
		print("Hit missed watermelon at: ", hit_pos)
